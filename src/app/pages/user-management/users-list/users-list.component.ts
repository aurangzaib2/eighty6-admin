...
export class UsersListComponent implements OnInit {
  ...
  constructor(config: NgbDropdownConfig, configModal: NgbModalConfig, private modalService: NgbModal,
    private userService: UserService, private spinner: SpinnerService, private toastService: ToastService, public translate: TranslateService) {
  ...
  tableOptions() {
    this.dtOptions = {
      ...
      columns: [{ data: 'username', orderable: false },
      ...
      ajax: (dataTablesParameters: any, callback) => {
        ...
        this.userService.getAllUsers(dataTablesParameters).subscribe(result => {
          this.spinner.stop();
  ...
  customizeRoleText(text): string {
    if (text === userRoles[1].role) {
      return userRoles[1].displayRole;
    }
   ...
   */
  add() {
    const modalRef = this.modalService.open(UserFormComponent, { centered: true });
