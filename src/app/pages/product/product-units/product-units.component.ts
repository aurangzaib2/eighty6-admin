...
export class ProductUnitsComponent implements OnInit {
  ...
  types = this.unitsForm.get('types') as FormArray;
 ...
 */
  ...
  getData() {
    this.spinner.start();
    this.productService.getUnits().subscribe(result => {
  ...
  */
  onSubmit() {
    ...
    if (this.types.value[0].id === null) {
      this.productService.addUnits(obj).subscribe(result => {
        this.spinner.stop();
   ...
   */
  update(obj) {
    this.productService.updateUnits(obj).subscribe(result => {
      this.spinner.stop();
      this.toastService.success(result);
      this.getData();
      this.selectedIndex = null;
  ...
  */
  openConfirmDelete(item, index) {
    ...
    modalRef.componentInstance.title = this.translate.instant('confirmMessages.deleteTitle')+` `+this.translate.instant('confirmMessages.unit');
    modalRef.componentInstance.description = this.translate.instant('confirmMessages.deleteDescription') +` `+ this.translate.instant('confirmMessages.unit')+ ` ?`;
  ...
  */
  deleteRequest(item) {
    this.spinner.start();
    this.productService.deleteUnits(item).subscribe(result => {
